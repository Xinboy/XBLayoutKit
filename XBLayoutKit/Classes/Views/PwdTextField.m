...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        if (!(CGRectIsEmpty(frame) || CGRectIsNull(frame))) {
            [self masonryWithNumberCount:self.numberCount NumberSpace:self.numberSpace];
        } else {
            [self frameWithNumberCount:self.numberCount NumberSpace:self.numberSpace];
        }
...
- (void)frameWithNumberCount:(CGFloat)count NumberSpace:(CGFloat)space {
    ...
    for (int i = 0; i < self.numberCount; i++) {
        ...
        bottomLine.frame = CGRectMake((self.numberSpace + lineWidth) * i, 39, lineWidth, 1);
        
        //Show number's label
        UILabel *numberLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(bottomLine.frame), CGRectGetHeight(self.frame))];
